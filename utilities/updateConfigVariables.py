...
Description:
    ...
    occurrences of ${BACKEND_IP}, ${PERS_BACKEND_IP}, ${DELPHI_BACKEND_IP} and ${BASE_DOMAIN} with the provided values. 
...
def update_file(filepath, BACKEND_IP, PERS_BACKEND_IP, DELPHI_BACKEND_IP, BASE_DOMAIN):
...
def process_conf_directory(directory, BACKEND_IP, PERS_BACKEND_IP, DELPHI_BACKEND_IP, BASE_DOMAIN):
    """Recursively process all .conf files in the given directory."""
    for root, dirs, files in os.walk(directory):
        for file in files:
            if file.endswith(".conf"):
                filepath = os.path.join(root, file)
                update_file(filepath, BACKEND_IP, PERS_BACKEND_IP, DELPHI_BACKEND_IP, BASE_DOMAIN)
...
def main():
    ...
    PERS_BACKEND_IP = prompt_input("PERS_BACKEND_IP", "Enter the backend IP address for your Persephone backups", last_values.get("PERS_BACKEND_IP"))
    DELPHI_BACKEND_IP = prompt_input("DELPHI_BACKEND_IP", "Enter the backend IP address for your Delphi install", last_values.get("DELPHI_BACKEND_IP"))
    ...
    new_values = {"BACKEND_IP": BACKEND_IP, "PERS_BACKEND_IP": PERS_BACKEND_IP, "DELPHI_BACKEND_IP": DELPHI_BACKEND_IP, "BASE_DOMAIN": BASE_DOMAIN}
    ...
    process_conf_directory(CONF_DIR, BACKEND_IP, PERS_BACKEND_IP, DELPHI_BACKEND_IP, BASE_DOMAIN)
