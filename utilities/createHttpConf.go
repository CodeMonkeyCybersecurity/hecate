...
// promptWithDefault displays a description and prompt with a default value,
// returning the user's input (or the default if no input is given).
func promptWithDefault(prompt, def, description string) string {
	...
	if err != nil {
		// In case of error, return the default.
		return def
	}
...
func main() {
	...
	// Prompt for configuration options with descriptions and defaults.
...
`, serverTokens, includeMime, defaultType, errorLogPath, errorLogLevel, accessLogPath, sendfile, includeServers)
	...
	// Check if http.conf exists and back it up.
